...
if ( '' === $term ) {
	...
	if ( ! $rows && ! $brand_rows ) {
		...
		foreach ( $rows as $r ) {
			...
			echo '<a class="tk-card" href="' . esc_url( $url ) . '"><strong>' . esc_html( $r->sslug . $r->size ) . '</strong><small>' . esc_html( $r->bname ) . '</small>' . ( null === $price ? '' : '<small>' . esc_html( tk_money_display( $price ) ) . '</small>' ) . '</a>';
		}
		foreach ( $brand_rows as $br ) {
			...
			echo '<a class="tk-card" href="' . esc_url( $url ) . '"><strong>' . esc_html( $br->sslug . $parsed['size'] ) . '</strong><small>' . esc_html( $br->bname ) . '</small>' . ( null === $price ? '' : '<small>' . esc_html( tk_money_display( $price ) ) . '</small>' ) . '</a>';
		}
		echo '</div>';
	}
}
